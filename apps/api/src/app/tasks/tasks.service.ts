...
addCron(data:any) {
  const targetDate = moment(data.time); // Your specific date
  const cronExpression = `0 ${targetDate.minute()} ${targetDate.hour()} ${targetDate.date()} ${targetDate.month() + 1} ${targetDate.isoWeekday()}`;
  ...
  if(Chinhanh)
  {
    ...
    const job = new CronJob(cronExpression, () => {
      console.log(`time (${data}) for job to run!`)
      this._ZaloznsService.sendtestzns(data,Chinhanh.id)
    })
